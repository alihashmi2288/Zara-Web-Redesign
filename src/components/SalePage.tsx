...
export default function SalePage() {
  ...
  const availableBrands = useMemo(() => {
    return Array.from(new Set(saleProducts.map((p) => p.brand).filter(Boolean))) as string[];
  }, [saleProducts]);

  const availableColors = useMemo(() => {
    return Array.from(new Set(saleProducts.flatMap((p) => p.colors.map((c) => c.name))));
  }, [saleProducts]);

  const availableSizes = useMemo(() => {
    return Array.from(new Set(saleProducts.flatMap((p) => p.sizes)));
  }, [saleProducts]);
    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="bg-black text-white py-24 px-6 text-center">
        <div className="max-w-4xl mx-auto space-y-4">
          <Reveal>
            <span className="text-[10px] tracking-[0.5em] font-bold text-red-500 uppercase">Special Promotion</span>
          </Reveal>
          <Reveal delay={0.1}>
          ...
          <Reveal delay={0.2}>
            <p className="text-xs md:text-sm tracking-widest text-gray-400 uppercase leading-relaxed">
              Enjoy discounts of up to 50% off select garments and accessories. Limited time only.
            </p>
          </Reveal>
        </div>
      </section>

      <div className="py-20 px-6 md:px-12 max-w-[1800px] mx-auto">
        <Reveal>
        ...
        <Reveal delay={0.1}>
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
            <div>
              <h2 className="text-2xl font-serif tracking-tight uppercase font-bold text-red-600">
                PROMOTIONS
              </h2>
              <p className="text-gray-500 mt-2 uppercase tracking-widest text-xs font-medium">
            ...
            {filteredProducts.map((product, index) => (
              ...
              >
                <ProductCard 
                  ...
                  tag={`${product.discountPercentage}% OFF`} 
                />
              </motion.div>
            ))}
          </AnimatePresence>
        ...
        {filteredProducts.length === 0 && (
          <div className="h-[40vh] flex flex-col items-center justify-center text-center">
            <p className="text-gray-400 uppercase tracking-widest text-sm">No promotional items found matching filters</p>
          </div>
        )}
      </div>
