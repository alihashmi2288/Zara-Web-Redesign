...
export default function BestSellersPage() {
  ...
  const availableBrands = useMemo(() => {
    return Array.from(new Set(bestProducts.map((p) => p.brand).filter(Boolean))) as string[];
  }, [bestProducts]);

  const availableColors = useMemo(() => {
    return Array.from(new Set(bestProducts.flatMap((p) => p.colors.map((c) => c.name))));
  }, [bestProducts]);

  const availableSizes = useMemo(() => {
    return Array.from(new Set(bestProducts.flatMap((p) => p.sizes)));
  }, [bestProducts]);

  const filteredProducts = useMemo(() => {
    let result = [...bestProducts];
      ...
      <div className="max-w-[1800px] mx-auto">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
            <div>
              ...
              <p className="text-gray-500 mt-4 uppercase tracking-widest text-xs font-medium">
                Showing {filteredProducts.length} top rated items
              </p>
            </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-4 gap-y-12 md:gap-x-8 md:gap-y-16">
          <AnimatePresence mode="popLayout">
            {filteredProducts.map((product, index) => (
              ...
              >
                <ProductCard product={product} tag="BEST SELLER" />
              </motion.div>
            ))}
          </AnimatePresence>
        </div>
        ...
        {filteredProducts.length === 0 && (
          <div className="h-[40vh] flex flex-col items-center justify-center text-center">
            <p className="text-gray-400 uppercase tracking-widest text-sm">No best seller items found matching selected filters</p>
          </div>
        )}
      </div>
